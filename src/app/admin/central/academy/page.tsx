    ...
    <div className="min-h-screen bg-[#0f172a] text-white p-8 font-sans relative">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        <div className="bg-[#1e293b] p-8 rounded-[40px] border border-slate-800 space-y-4">
           <h3 className="text-xs font-black uppercase italic mb-4">Novo Vídeo/Material</h3>
           <input className="w-full bg-[#0f172a] p-4 rounded-2xl border border-slate-800 text-white outline-none focus:border-purple-500" placeholder="Título da Aula" value={novoCurso.titulo} onChange={e=>setNovoCurso({...novoCurso, titulo: e.target.value})} />
           <input className="w-full bg-[#0f172a] p-4 rounded-2xl border border-slate-800 text-white outline-none focus:border-purple-500" placeholder="ID YouTube" value={novoCurso.link} onChange={e=>setNovoCurso({...novoCurso, link: e.target.value})} />
           ...
           <button onClick={salvar} disabled={loading} className="w-full bg-purple-600 p-4 rounded-2xl font-black uppercase text-xs tracking-widest">{loading ? 'Publicando...' : 'Publicar Conteúdo'}</button>
        </div>
        <div className="bg-[#1e293b] p-8 rounded-[40px] border border-slate-800">
           ...
           <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
              {cursos.map(c => (
                <div key={c.id} className="bg-[#0f172a] p-4 rounded-2xl border border-slate-800 flex justify-between items-center group hover:border-purple-500/50">
                  <span className="font-bold text-sm uppercase italic">{c.titulo}</span>
                  <button onClick={() => apagar(c.id)} className="text-slate-600 hover:text-red-500 transition-colors">🗑️</button>
                </div>
              ))}
           </div>
        </div>

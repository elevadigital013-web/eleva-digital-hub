...
export default function VendedorDashboard() {
  ...
  useEffect(() => {
    async function loadData() {
      ...
      // 2. Busca leads DO VENDEDOR LOGADO
      ...
      if (leads) {
        ...
        leads.forEach(l => {
          // Verifica se é fechado (independente de maiúscula/minúscula)
          if (l.status && l.status.toLowerCase() === 'fechado') {
            fechados++;
            const v = Number(l.valor_venda) || 0;
            valorTotal += v;
            
            // Regra de comissão (25% acima de 5k, se não 20%)
            const perc = v >= 5000 ? 0.25 : 0.20;
            if (!l.pago) comissaoTotal += (v * perc);
          } else {
            abertos++; // Considera como lead em aberto
          }
        });
    ...
    <div className="min-h-screen bg-slate-50 p-6 font-sans">
      ...
      <div className="flex justify-between items-center mb-8">
        ...
        <button 
            ...
            className="bg-white p-3 rounded-full shadow-sm text-red-500 border border-slate-100 hover:bg-red-50 transition-colors"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        {/* CARD 3: PLACAR (FECHADAS vs ABERTAS) */}
